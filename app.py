...
USER_SMS = "12026027"
API_KEY_SMS = "MF7Qjs3C8KxKHz"
NUMERO_GERANT = "+33617722379"
...
def liste_clients(df):
    ...
    if not data.empty:
        ...
        total = data[["prix_brut", "prix_net", "charges", "nuitees"]].sum().to_frame().T
        st.markdown("#### Totaux")
        st.dataframe(total)
        ...
        buffer.seek(0)
        st.download_button("📥 Télécharger Liste Clients", data=buffer, file_name=f"clients_{annee}_{mois}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    else:
        st.info("Aucune donnée pour cette période.")
...
def envoyer_sms(numero, message):
    ...
    params = {"user": USER_SMS, "pass": API_KEY_SMS, "msg": message}
    response = requests.get(url, params=params)
    return response.status_code == 200

def bouton_envoi_sms(df):
    st.subheader("📩 SMS à envoyer aujourd’hui pour les arrivées de demain")
    demain = (date.today() + timedelta(days=1))
    df_notif = df[df["date_arrivee"] == demain]
    if df_notif.empty:
        st.info("Aucun SMS à envoyer pour demain.")
        return
    for _, row in df_notif.iterrows():
        ...
        message = f"VILLA TOBIAS - {plateforme}\nBonjour {nom}. Votre séjour est prévu du {arrivee} au {depart}. Afin de vous accueillir merci de nous confirmer votre heure d’arrivée. Un parking est disponible. À demain."

        if st.button(f"📤 Envoyer à {nom}"):
            envoyer_sms(NUMERO_GERANT, message)
            log = {"date": date.today(), "client": nom, "message": message}
            if os.path.exists(SMS_HISTORIQUE):
                hist = pd.read_csv(SMS_HISTORIQUE)
                hist = pd.concat([hist, pd.DataFrame([log])], ignore_index=True)
            else:
                hist = pd.DataFrame([log])
            hist.to_csv(SMS_HISTORIQUE, index=False)
...
def main():
    df = charger_donnees()
    onglet = st.sidebar.radio("Menu", ["📋 Réservations", "➕ Ajouter", "✏️ Modifier / Supprimer", "📅 Calendrier", "📊 Rapport", "📇 Liste clients", "📩 SMS à envoyer", "🗂️ Historique SMS"])
