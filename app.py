...
def liste_clients(df):
    ...
    if data.empty:
        ...
        return
    colonnes = ["nom_client", "date_arrivee", "date_depart", "nuitees", "prix_brut", "prix_net", "plateforme"]
    data_affichee = data[colonnes].copy()
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        data_affichee.to_excel(writer, index=False)
    buffer.seek(0)
    st.download_button("📥 Télécharger liste Excel", data=buffer, file_name=f"clients_{annee}_{mois}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
