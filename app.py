...
import os
from send_sms import envoyer_sms_free

FICHIER = "reservations.xlsx"
FICHIER_SMS = "sms_log.csv"
...
def afficher_calendrier(df):
    ...
    for semaine in calendar.monthcalendar(annee, mois_index):
        ligne = []
        for jour in semaine:
            jour_date = date(annee, mois_index, jour) if jour != 0 else ""
            contenu = f"{jour}\n" + "\n".join(planning.get(jour_date, [])) if jour else ""
...
def liste_clients(df):
    ...
    mois = st.selectbox("Mois", ["Tous"] + list(calendar.month_name)[1:], key="mois_liste")
    data = df[df["annee"] == annee]
    if mois != "Tous":
        data = data[data["mois"] == list(calendar.month_name).index(mois)]
    ...
    if not data.empty:
        total = data[colonnes[4:]].sum(numeric_only=True).round(2)
        st.write("**Total :**")
        st.write(total.to_frame().T)
        csv = data[colonnes].to_csv(index=False).encode("utf-8")
        st.download_button("📥 Télécharger CSV", data=csv, file_name="liste_clients.csv", mime="text/csv")
...
def notifier_arrivees_prochaines(df):
    ...
    for _, row in df_notif.iterrows():
        ...
        envoyer_sms_free(row["telephone"], message)
        ajouter_sms_historique(row["telephone"], message)

# 📥 Historique SMS
def ajouter_sms_historique(numero, message):
    date_envoi = datetime.now().strftime("%Y-%m-%d %H:%M")
    ligne = pd.DataFrame([[date_envoi, numero, message]], columns=["date", "numero", "message"])
...
def afficher_historique_sms():
    ...
    if os.path.exists(FICHIER_SMS):
        ...
        st.download_button("📥 Télécharger CSV", data=csv, file_name="historique_sms.csv", mime="text/csv")
    else:
...
def main():
    ...
    onglet = st.sidebar.radio("Navigation", [
        ...
        "📅 Calendrier", "📊 Rapport", "📋 Liste clients", "📨 SMS envoyés"
    ])
